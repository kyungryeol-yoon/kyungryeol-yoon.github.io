...
> 1명의 가상 유저가 한번만 default function을 호출하는 것이 아닌, 60초 동안 해당 함수를 계속 호출한다.\\즉, default function을 1명씩 1번 호출해서 총 10번 호출하는 것이 아닌, 1명의 가상유저가 5~10번 정도 호출한다.
{: .prompt-info }

> Production 환경이 아닌 Dev 또는 Staging 환경에서 진행\\성능을 분석할 때, 절대 평균값으로 판단하지 않기\\클라우드 환경을 이용한다면, k6를 테스트할 때 Scale Up, Out에 주의
{: .prompt-warning }
