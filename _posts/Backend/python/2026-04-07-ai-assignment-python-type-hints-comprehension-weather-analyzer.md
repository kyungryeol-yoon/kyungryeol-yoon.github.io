...
첫 번째 과제는 **타입 힌트**와 **컴프리헨션**을 활용한 날씨 데이터 분석 클래스 만들기입니다.
...
**타입 힌트 오류:** `dict[str]`은 유효하지 않습니다. `dict`는 key와 value 타입을 둘 다 명시해야 합니다. 값이 `str`, `float`, `int` 등 여러 타입이 섞여 있으므로 `dict[str, Any]`로 써야 합니다.

**get_city_avg 로직 오류:** 가장 큰 실수였습니다. 온도, 습도, 미세먼지를 **합산해서 3으로 나누는** 의미 없는 계산을 하고 있었습니다. 요구사항은 각 항목의 **도시별 평균**이었습니다.
...
**딕셔너리 키 중복 문제:** 컴프리헨션에서 같은 키("부산")가 여러 번 나오면 마지막 값만 남습니다. 즉 부산 4/1 데이터가 사라집니다.

**summary의 반환 타입 불일치:** 리스트로 감싸서 `list[dict]`를 반환하고 있었는데, 요구사항은 `dict[str, dict]`이었습니다.
...
- 항목별 평균을 **따로** 구하는 방향은 맞았습니다.
- `summary()`에서 `get_city_avg()`를 **재활용**하는 설계도 좋았습니다.

### 여전히 남은 문제

**@dataclass 오용:** `@dataclass`는 `__init__`을 자동 생성해주는 데코레이터인데, 직접 `__init__`을 정의하면 자동생성이 무시됩니다. 위에 선언한 `city`, `date` 등의 필드도 실제로 사용되지 않았습니다.

**반환 구조:** `dict` 안에 `list` 안에 `dict` 3개라는 불필요하게 복잡한 구조가 만들어졌습니다.
...
**같은 필터링이 6번 반복:** `data['city'] == city` 필터링을 temp, humidity, pm25마다 sum과 len에서 각각 반복하고 있었습니다. 총 6번. 먼저 한 번 필터링하고 재사용하면 됩니다.
...
리뷰에서 받은 모범답안에는 몇 가지 추가 테크닉이 있었습니다.
...
`FIELDS`를 상수로 정의해두면 필드가 추가되어도 한 곳만 수정하면 됩니다. 딕셔너리 컴프리헨션으로 3줄이 1줄로 줄어듭니다.
...
> ⚠️ 존재하지 않는 도시를 넣으면 `n = 0`이 되어 `ZeroDivisionError`가 발생합니다. 실무에서는 이런 엣지 케이스를 항상 고려해야 합니다.
...
`print(analyzer)` 했을 때 객체 정보를 한눈에 볼 수 있어 디버깅에 유용합니다.

---

## 📝 이번 과제에서 배운 것

**타입 힌트:** `dict[str]`이 아니라 `dict[str, Any]`처럼 key/value 타입을 모두 명시해야 합니다. 모던 Python에서 타입 힌트는 선택이 아닌 사실상 표준입니다.

**컴프리헨션의 함정:** 딕셔너리 컴프리헨션에서 같은 키가 여러 번 나오면 마지막 값만 남습니다. 데이터를 먼저 그룹핑하고 집계해야 합니다.

**@dataclass의 용도:** `@dataclass`는 데이터를 담는 클래스에 쓰는 것이지, 로직 중심의 분석 클래스에 억지로 끼워넣을 필요가 없습니다.

**필터링 한 번, 재사용 여러 번:** 같은 조건의 필터링을 반복하지 말고 변수에 담아서 재사용합니다. 가독성과 성능 모두 좋아집니다.

> **Tip**: 메서드 안에 `print`를 넣지 않는 것이 좋은 습관입니다. 메서드는 값을 반환하고, 출력은 호출하는 쪽에서 결정해야 합니다.
