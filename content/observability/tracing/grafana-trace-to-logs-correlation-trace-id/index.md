...
그리고 이 트레이스 확장은 [**"OTel + VictoriaLogs 로그 스택" 시리즈**(완결)](/observability/opentelemetry/otel-collector-agent-gateway-architecture/) 위에 얹힌 것입니다. 둘을 합치면 **로그(수집·저장·조회) + 트레이스(수집·저장·조회) + 연결 = 통합 관측성**이 됩니다.
...
> 🔗 함께 보기: [**"OTel + VictoriaLogs 로그 스택" 시리즈**(완결)](/observability/opentelemetry/otel-collector-agent-gateway-architecture/) — 로그 스택 전체.
...
- 관련 글: [Grafana에 VictoriaLogs 연결하기 (로그 스택 대시보드 트랙)](/observability/opentelemetry/grafana-victorialogs-datasource-explore-dashboard/)
