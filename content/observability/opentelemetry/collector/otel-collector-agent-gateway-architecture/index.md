...
> 💡 순서가 중요합니다. `k8sattributes`만 켜고 `filelog`의 경로 추출(`include_file_path: true` + `container` operator)이 없으면, 기준이 되는 pod 정보 자체가 없어 **k8s 필드가 하나도 안 붙습니다.** Helm 차트에서는 `logsCollection` 프리셋이 이 둘을 포함한 `filelog`를 자동 구성하므로 함께 동작합니다(설치·수동 구성 예시는 [3편](/observability/opentelemetry/collector/kubernetes-otel-collector-offline-helm-install/) 참고).
