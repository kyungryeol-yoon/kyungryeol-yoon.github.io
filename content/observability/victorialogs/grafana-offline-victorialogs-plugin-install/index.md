...
> ⚠️ **`url` 포트는 VictoriaLogs 배포 모드에 따라 다릅니다.** `9428`은 **single-node** 값이고, **클러스터 모드는 vmauth 진입 `8427`**(`http://<release>-vmauth.<ns>.svc:8427`)을 씁니다. 조회 URL 상세는 [Grafana 데이터소스 연결 편](/observability/victorialogs/grafana-victorialogs-datasource-explore-dashboard/)을, 컴포넌트별 포트는 [VictoriaLogs 클러스터 구축 편](/observability/victorialogs/kubernetes-victorialogs-cluster-helm-install/#-victorialogs-클러스터는-무엇으로-구성되나)을 참고하세요.
