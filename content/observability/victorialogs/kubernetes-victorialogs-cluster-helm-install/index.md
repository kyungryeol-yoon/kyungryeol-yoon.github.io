...
VictoriaLogs 클러스터는 **vlinsert(수집)·vlstorage(저장)·vlselect(쿼리)** 와 진입점 **vmauth** 로 구성되며, 앞의 셋은 **같은 바이너리의 역할 분리**입니다. Helm 차트로 **retention·PVC 크기·vlstorage replica·vmauth** 네 가지만 정하면 대규모 로그 백엔드가 완성됩니다. 모든 적재·쿼리·vmui 접근은 **vmauth(8427) 단일 진입점**으로 통하며, 백엔드 포트는 외부에 직접 노출하지 않습니다. 이 글은 **"OTel + VictoriaLogs 로그 스택" 시리즈 2편(백엔드편)** 으로, [1편(개념편)](/observability/opentelemetry/collector/otel-collector-agent-gateway-architecture/)이 "VictoriaLogs로 보낸다"고 전제했던 **바로 그 백엔드를 실제로 세우는** 글입니다. **백엔드를 먼저 세우고 수집기를 붙이는** 순서이므로, [3편(OTel 설치편)](/observability/opentelemetry/collector/kubernetes-otel-collector-offline-helm-install/) 직전에 읽으면 좋습니다.
...
> ⚠️ vmauth `url_map`을 **직접 커스텀**한다면 두 함정을 주의하세요 — `url_prefix` 끝에 `/insert`·`/select`를 붙이면 경로가 중복되어 `400`이 나고, 포괄 규칙(`/.*`) 하나로 전부 vlinsert로 보내면 조회가 `-select.disable`에 걸립니다. 올바른 `/insert/.*`→vlinsert, `/select/.*`→vlselect 분리 설정은 [트러블슈팅편 — vmauth 400](/observability/victorialogs/victorialogs-otel-troubleshooting/#-2-vmauth-400-url_prefix-경로-중복) · [select.disable](/observability/victorialogs/victorialogs-otel-troubleshooting/#-3--selectdisable-에러-조회가-vlinsert로-잘못-라우팅)을 참고하세요.
...
수집기 설치는 [OTel 설치편](/observability/opentelemetry/collector/kubernetes-otel-collector-offline-helm-install/), Grafana 플러그인 연결은 [VictoriaLogs 데이터소스 설치 글](/observability/victorialogs/grafana-offline-victorialogs-plugin-install/)에서 이어집니다.
...
- 관련 글: [폐쇄망 OTel Collector Helm 설치 (수집기 설치편)](/observability/opentelemetry/collector/kubernetes-otel-collector-offline-helm-install/)
