...
Kong 쪽은 stream 리스너 `:9443 ssl`을 열고 Route에 `protocols: [tls]` + `snis`를 설정해 TLS를 종단한 뒤 백엔드로 분배합니다 — 이 부분은 [Kong으로 TCP/TLS 서비스 외부 노출하기](/kubernetes/networking/gateway-api/kong-gateway-tcp-tls-sni-loadbalancer/) 글을 참고하세요.
...
- 관련 글: [Kong으로 TCP/TLS 서비스 외부 노출하기 (LoadBalancer + SNI)](/kubernetes/networking/gateway-api/kong-gateway-tcp-tls-sni-loadbalancer/) · [Ingress에서 Gateway API HTTPRoute로](/kubernetes/networking/gateway-api/kubernetes-ingress-to-gateway-api-httproute/)
