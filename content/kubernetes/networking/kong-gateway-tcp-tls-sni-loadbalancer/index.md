...
```text
클라이언트 ──(TLS + SNI)──▶ LoadBalancer(외부 IP) ──▶ Kong(stream 리스너, ssl)
                                                          └─(SNI로 분배)─▶ redis-a / redis-b / db-1 ...
```
...
> HTTP(S) 서비스라면 이 글이 아니라 [Ingress → Gateway API HTTPRoute 전환](/kubernetes/networking/kubernetes-ingress-to-gateway-api-httproute/)이나 [Gateway API HTTP→HTTPS 리다이렉트](/kubernetes/networking/kubernetes-gateway-api-http-to-https-redirect/)를 참고하세요.
...
Kong 설치 자체가 처음이라면 [Kong Ingress Controller 설치 (Gateway API)](/kubernetes/networking/kubernetes-install-kong-ingress-controller/)를 먼저 보고 오세요.
...
- 인증서 Secret은 [cert-manager](/kubernetes/networking/kubernetes-cert-manager-pki-tls/)로 발급·관리하면 편합니다.
...
**"어느 홉에서 깨졌나"를 격리로 좁히는 것**이 가장 빠릅니다.
...
| 상황 | 선택 |
|---|---|
| HTTP/HTTPS API·웹 | **HTTPRoute** (이 글 아님 → [HTTPRoute 전환 글](/kubernetes/networking/kubernetes-ingress-to-gateway-api-httproute/)) |
...
| gRPC의 L4/L7 로드밸런싱이 고민 | [gRPC L4/L7 로드밸런싱 글](/kubernetes/networking/kubernetes-grpc-l4-l7-load-balancing/) |
...
- 관련 글: [Kong Ingress Controller 설치 (Gateway API)](/kubernetes/networking/kubernetes-install-kong-ingress-controller/)
- 관련 글: [Ingress → Gateway API HTTPRoute 전환](/kubernetes/networking/kubernetes-ingress-to-gateway-api-httproute/)
- 관련 글: [Gateway API로 HTTP→HTTPS 리다이렉트](/kubernetes/networking/kubernetes-gateway-api-http-to-https-redirect/)
- 관련 글: [Kubernetes cert-manager로 PKI·TLS 인증서 관리](/kubernetes/networking/kubernetes-cert-manager-pki-tls/)
