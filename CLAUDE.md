...
### 모드 C: 유사 주제 글 병합 (재탄생)
비슷한 주제로 나뉜 글들을 **완전히 새로운 종합 글 1개로 재작성**하고 원본은 삭제합니다.
1. 대상 글들의 내용을 모두 읽고, **삭제 시 깨질 인바운드 링크** 확인(다른 글이 그 slug를 링크하는지).
2. **각 방법·접근을 방법별 섹션으로 보존** — 절차·코드·매니페스트를 **줄이거나 빼지 않음**(예: `## 1️⃣ 방법 A`, `## 2️⃣ 방법 B`). "여러 방법이 있다"는 기록 의도를 살림.
3. **웹 조사로 보강**(`/write`처럼): 최신 옵션·베스트프랙티스·주의점 추가, 원본의 오류(예: 깨진 YAML 들여쓰기)는 바로잡음.
4. AEO 구조 적용: 상단 요약 · 🆚 비교표 · `## 🤔 언제 무엇을` 선택 가이드 · `## ❓ 자주 묻는 질문` · `## 📚 참고`(출처).
5. 새 글 생성 후 **원본 폴더 삭제**. URL 리다이렉트가 필요할 때만 새 글에 `aliases: [구 URL]` 추가(기본은 미설정).
